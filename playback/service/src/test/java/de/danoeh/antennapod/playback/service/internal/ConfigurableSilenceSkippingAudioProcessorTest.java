...
public class ConfigurableSilenceSkippingAudioProcessorTest {
    ...
    public void testStrengthDurationsAndRatios() {
        assertEquals(160_000L, ConfigurableSilenceSkippingAudioProcessor.getMinimumSilenceDurationUs(1));
        assertEquals(0.35f, ConfigurableSilenceSkippingAudioProcessor.getSilenceRetentionRatio(1), 0.001f);

        assertEquals(140_000L, ConfigurableSilenceSkippingAudioProcessor.getMinimumSilenceDurationUs(2));
        assertEquals(0.30f, ConfigurableSilenceSkippingAudioProcessor.getSilenceRetentionRatio(2), 0.001f);

        assertEquals(120_000L, ConfigurableSilenceSkippingAudioProcessor.getMinimumSilenceDurationUs(3));
        assertEquals(0.25f, ConfigurableSilenceSkippingAudioProcessor.getSilenceRetentionRatio(3), 0.001f);
    ...
    @Test
    public void testSkippingSilence() throws Exception {
        processor.setEnabled(true);
        processor.configure(AUDIO_FORMAT);
        processor.flush();

        ByteBuffer silence = ByteBuffer.allocateDirect(44100 * 4 * 2).order(ByteOrder.nativeOrder());
        while (silence.hasRemaining()) {
        ...
        assertTrue(processor.getSkippedFrames() > 0);
    }
}
